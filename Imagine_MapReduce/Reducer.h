    ...
    class MasterNode{
        public:
            ...
            void MemoryMerge()
            {
                ...
                int merge_num=memory_file_list.size();
                for(auto it=memory_file_list.begin();it!=memory_file_list.end();it++){
                    merge_list.push_back(std::move(*it));
                }
                memory_file_list.clear();

                std::string merge_name="memory_merge_"+MapReduceUtil::IntToString(memory_merge_id++)+".txt";
                ...
                for(int i=0;i<merge_num;i++){
                    std::string key_;
                    std::string value_;
                    if(MapReduceUtil::ReadKVReaderFromMemory(merge_list[i],idxs[i],key_,value_))heap_.push(new KVReader(key_,value_,i));
                }
                while(heap_.size()){
                    KVReader* next_kv=heap_.top();
                    heap_.pop();
                    std::string key_;
                    std::string value_;
                ...
                disk_file_list.push_back(merge_name);
        ...
        public:
            ...
            int file_num;
            std::unordered_map<std::string,std::unordered_map<std::string,int>> files;//存储源文件名到splits的映射
            ...
            pthread_mutex_t* memory_list_lock;
            pthread_mutex_t* disk_list_lock;
            std::atomic<int> memory_file_size;//存储内存中储存的文件的总大小
            // int memory_file_size;//存储内存中储存的文件的总大小
            std::list<std::string> memory_file_list;//存储split文件内容
            std::list<std::string> disk_file_list;//存储磁盘中储存的文件的文件名
    };
...
private:
    ...
    std::unordered_map<std::pair<std::string,std::string>,MasterNode*,HashPair,EqualPair> master_map;
...
std::vector<std::string> Reducer<key,value>::Reduce(const std::vector<std::string>& input)
{
    ...
    */
    ...
    pthread_mutex_lock(map_lock);
    typename std::unordered_map<std::pair<std::string,std::string>,MasterNode*,HashPair,EqualPair>::iterator master_it=master_map.find(master_pair);//找到MasterNode*
    ...
    master_it=master_map.find(master_pair);
    file_it=master_it->second->files.find(file_name);
    if(file_it->second.size()==split_num)master_it->second->count++;
