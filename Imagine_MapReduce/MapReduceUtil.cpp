...
bool MapReduceUtil::ReadKVReaderFromMemory(const std::string& content, int& idx, std::string& key_, std::string& value_)//从spill文件读取kv对
{
    ...
    int start_idx=idx;
    while(content[++idx]!=' ');
    key_=content.substr(start_idx,idx-start_idx);
...
bool MapReduceUtil::WriteKVReaderToDisk(const int fd, const KVReader* const kv_reader)
{
    char c=' ';
    char cc[3]="\r\n";
    write(fd,&kv_reader->reader_key[0],kv_reader->reader_key.size());
    write(fd,&c,1);
    write(fd,&kv_reader->reader_value[0],kv_reader->reader_value.size());
    write(fd,cc,2);
